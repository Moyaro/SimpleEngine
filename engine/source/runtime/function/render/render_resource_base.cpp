...
namespace SimpleEngine {
    std::shared_ptr<TextureData> RenderResourceBase::loadTexture(std::string file, bool is_srgb)
    {
        ...
        int width, height;
        texture->m_pixels = stbi_load(asset_manager->getFullPath(file).generic_string().c_str(), &width, &height, 0, STBI_rgb_alpha);

        if (!texture->m_pixels) return nullptr;
        ...
        texture->m_type = ENGINE_IMAGE_TYPE::ENGINE_IMAGE_TYPE_2D;
    ...
    std::shared_ptr<TextureData> RenderResourceBase::loadTextureHDR(std::string file, int desired_channels)
    {
        ...
        {
        case 2:
            texture->m_format = VK_FORMAT_R32G32_SFLOAT;
            break;
        case 4:
    ...
    StaticMeshData RenderResourceBase::loadStaticMesh(std::string filename, AxisAlignedBox& bounding_box)
    {
        ...
        if (!reader.ParseFromFile(filename, reader_config))
        {
            ...
            {
                std::cout<<"loadMesh failed, error: "<<filename<<":" << reader.Error() << std::endl;
            }
            assert(0);
        }
        ...
        {
            std::cout << "loadMesh failed, warning: " << filename << ":" << reader.Warning() << std::endl;
        }
