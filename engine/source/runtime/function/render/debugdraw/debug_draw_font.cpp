...
#include "../interface/vulkan_util.h"
...
namespace SimpleEngine {
    ...
    void DebugDrawFont::loadFont()
    {
        ...
        stbi_uc* pixels = stbi_load(g_runtime_global_context.m_config_manager->getTestPicturePath().generic_string().c_str(), &tex_width, &tex_height, &tex_channels,STBI_rgb_alpha);
        std::shared_ptr<RHI> rhi = g_runtime_global_context.m_render_system->getRHI();
        rhi->createGlobalImage(m_font_image, m_font_imageView, m_allocation, tex_width, tex_height, pixels, VK_FORMAT_R8G8B8A8_UNORM);*/
        ...
        if (fontFile == NULL)
        {
            std::runtime_error("无法打开 font.ttf");
        }
        ...
        std::shared_ptr<RHI> rhi = g_runtime_global_context.m_render_system->getRHI();
        rhi->createGlobalImage(m_font_image, m_font_imageView, m_allocation, m_bitmap_w, m_bitmap_h, imageData.data(), VK_FORMAT_R32_SFLOAT);
