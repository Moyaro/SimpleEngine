...
{
    void PointLightShadowPass::init(const RenderPassInitInfo* init_info)
    {
    ...
    void PointLightShadowPass::setupPipelines()
    {
        ...
        VkShaderModule vert_shader_module = m_rhi->createShaderModule(MESH_POINT_LIGHT_SHADOW_VERT);
        VkShaderModule geom_shader_module = m_rhi->createShaderModule(MESH_POINT_LIGHT_SHADOW_GEOM);
        VkShaderModule frag_shader_module = m_rhi->createShaderModule(MESH_POINT_LIGHT_SHADOW_FRAG);
    ...
    void PointLightShadowPass::setupDescriptorSet()
    {
        ...
        if (VK_SUCCESS != m_rhi->allocateDescriptorSets(&mesh_point_light_shadow_global_descriptor_set_alloc_info, &m_descriptor_infos[0].descriptor_set))
        {
            ...
            for (auto& pair1 : point_lights_mesh_drawcall_batch)
            {
                ...
                for (auto& pair2 : mesh_instanced)
                {
                    ...
                    if (total_instance_count > 0)
                    {
                        ...
                        m_rhi->cmdBindVertexBuffers(
                            m_rhi->getCurrentCommandBuffer(), 0, 1, vertex_buffers, offsets);
                        m_rhi->cmdBindIndexBuffer(
                            m_rhi->getCurrentCommandBuffer(), mesh.mesh_index_buffer, 0, VK_INDEX_TYPE_UINT16);
