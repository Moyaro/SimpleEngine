...
{
    void DirectionalLightShadowPass::init(const RenderPassInitInfo* init_info)
    {
    ...
    void DirectionalLightShadowPass::setupRenderPass()
    {
        ...
        if (VK_SUCCESS != m_rhi->createRenderPass(&renderpass_create_info, &m_framebuffer.render_pass))
        {
    ...
    void DirectionalLightShadowPass::setupFramebuffer()
    {
        ...
        if (VK_SUCCESS != m_rhi->createFramebuffer(&framebuffer_create_info, &m_framebuffer.framebuffer))
        {
    ...
    void DirectionalLightShadowPass::setupDescriptorSetLayout()
    {
        ...
        if (VK_SUCCESS != m_rhi->createDescriptorSetLayout(&mesh_point_light_shadow_global_layout_create_info, &m_descriptor_infos[0].layout))
        {
    ...
    void DirectionalLightShadowPass::setupPipelines()
    {
        ...
        if (VK_SUCCESS != m_rhi->createPipelineLayout(&pipeline_layout_create_info, &m_render_pipelines[0].layout))
        {
            throw std::runtime_error("create mesh directional light shadow pipeline layout");
        }

        VkShaderModule vert_shader_module = m_rhi->createShaderModule(MESH_DIRECTIONAL_LIGHT_SHADOW_VERT);
        VkShaderModule frag_shader_module = m_rhi->createShaderModule(MESH_DIRECTIONAL_LIGHT_SHADOW_FRAG);
        ...
        if (VK_SUCCESS != m_rhi->createGraphicsPipelines(VK_NULL_HANDLE, 1, &pipelineInfo, &m_render_pipelines[0].pipeline))
        {
    ...
    void DirectionalLightShadowPass::setupDescriptorSet()
    {
        ...
        if (VK_SUCCESS != m_rhi->allocateDescriptorSets(&mesh_directional_light_shadow_global_descriptor_set_alloc_info, &m_descriptor_infos[0].descriptor_set))
        {
        ...
        mesh_directional_light_shadow_perframe_storage_buffer_write_info.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
        ...
        VkWriteDescriptorSet& mesh_directional_light_shadow_perdrawcall_storage_buffer_write_info = descriptor_writes[1];
        mesh_directional_light_shadow_perdrawcall_storage_buffer_write_info.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        ...
        mesh_directional_light_shadow_perdrawcall_storage_buffer_write_info.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
            ...
            for (auto& [material, mesh_instanced] : directional_light_mesh_drawcall_batch)
            {
                ...
                for (auto& [mesh, mesh_nodes] : mesh_instanced)
                {
                    ...
                    if (total_instance_count > 0)
                    {
                        ...
                        m_rhi->cmdBindVertexBuffers(m_rhi->getCurrentCommandBuffer(), 0, 1, vertex_buffers, offsets);
