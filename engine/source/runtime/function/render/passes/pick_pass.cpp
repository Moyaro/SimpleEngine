...
{
    void PickPass::init(const RenderPassInitInfo* init_info)
    {
    ...
    {
        VkImageView attachments[2] = {m_framebuffer.attachments[0].view, m_rhi->getDepthImageInfo().depth_image_view };
        ...
        if (m_rhi->createFramebuffer(&framebuffer_create_info, &m_framebuffer.framebuffer) != VK_SUCCESS)
        {
    ...
    void PickPass::setupDescriptorSetLayout()
    {
        ...
        VkDescriptorSetLayoutBinding& mesh_inefficient_pick_global_layout_perframe_storage_buffer_binding = mesh_inefficient_pick_global_layout_bindings[0];
        mesh_inefficient_pick_global_layout_perframe_storage_buffer_binding.binding = 0;
        mesh_inefficient_pick_global_layout_perframe_storage_buffer_binding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
        ...
        VkDescriptorSetLayoutBinding& mesh_inefficient_pick_global_layout_perdrawcall_storage_buffer_binding = mesh_inefficient_pick_global_layout_bindings[1];
        mesh_inefficient_pick_global_layout_perdrawcall_storage_buffer_binding.binding = 1;
        mesh_inefficient_pick_global_layout_perdrawcall_storage_buffer_binding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
        ...
        if (VK_SUCCESS != m_rhi->createDescriptorSetLayout(&mesh_inefficient_pick_global_layout_create_info, &m_descriptor_infos[0].layout))
    ...
    void PickPass::setupPipelines()
    {
        ...
        if (m_rhi->createPipelineLayout(&pipeline_layout_create_info, &m_render_pipelines[0].layout) != VK_SUCCESS)
        {
            throw std::runtime_error("create mesh inefficient pick pipeline layout");
        }

        VkShaderModule vert_shader_module = m_rhi->createShaderModule(MESH_INEFFICIENT_PICK_VERT);
        VkShaderModule frag_shader_module = m_rhi->createShaderModule(MESH_INEFFICIENT_PICK_FRAG);
        ...
        if (m_rhi->createGraphicsPipelines(VK_NULL_HANDLE, 1, &pipelineInfo, &m_render_pipelines[0].pipeline) != VK_SUCCESS)
        {
    ...
    void PickPass::setupDescriptorSet()
    {
        ...
        if (VK_SUCCESS != m_rhi->allocateDescriptorSets(&mesh_inefficient_pick_global_descriptor_set_alloc_info, &m_descriptor_infos[0].descriptor_set))
        {
    ...
    uint32_t PickPass::pick(const Vector2& picked_uv)
    {
        ...
        bool res_begin_command_buffer = m_rhi->beginCommandBuffer(
        ...
        m_rhi->cmdBeginRenderPass(m_rhi->getCurrentCommandBuffer(),
        ...
        uint32_t perframe_dynamic_offset = roundUp(m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()],m_global_render_resource->_storage_buffer._min_storage_buffer_offset_alignment);
        m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = perframe_dynamic_offset + sizeof(MeshInefficientPickPerframeStorageBufferObject);
        ...
        for (auto& pair1 : main_camera_mesh_drawcall_batch)
        {
            ...
            for (auto& pair2 : mesh_instanced)
            {
                ...
                {
                    // bind per mesh
                    m_rhi->cmdBindDescriptorSets(m_rhi->getCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_render_pipelines[0].layout, 1, 1, &mesh.mesh_vertex_blending_descriptor_set, 0, nullptr);
        ...
        // end command buffer
        bool res_end_command_buffer = m_rhi->endCommandBuffer(m_rhi->getCurrentCommandBuffer());
        assert(VK_SUCCESS == res_end_command_buffer);

        bool res_reset_fences = m_rhi->resetFences(1, &m_rhi->getFenceList()[m_rhi->getCurrentFrameIndex()]);
        ...
        VkCommandBuffer commandBuffer = m_rhi->getCurrentCommandBuffer();
        ...
        bool res_wait_for_fences = m_rhi->waitForFences(m_rhi->getMaxFramesInFlight(),
                                                           m_rhi->getFenceList(),
            VK_TRUE,
                                                           UINT64_MAX);
