...
{
    void CombineUIPass::init(const RenderPassInitInfo* init_info)
    {
    ...
    void CombineUIPass::setupDescriptorSetLayout()
    {
        ...
        if (VK_SUCCESS != m_rhi->createDescriptorSetLayout(&post_process_global_layout_create_info, &m_descriptor_infos[0].layout))
        {
    ...
    void CombineUIPass::setupPipelines()
    {
        ...
        VkShaderModule vert_shader_module = m_rhi->createShaderModule(POST_PROCESS_VERT);
        VkShaderModule frag_shader_module = m_rhi->createShaderModule(COMBINE_UI_FRAG);
    ...
    void CombineUIPass::setupDescriptorSet()
    {
        ...
        if (VK_SUCCESS != m_rhi->allocateDescriptorSets(&post_process_global_descriptor_set_alloc_info, &m_descriptor_infos[0].descriptor_set))
        {
    ...
    void CombineUIPass::draw()
    {
        ...
        VkViewport viewport = {0.0,
                               ...
                               0.0,
                               1.0};
        VkRect2D   scissor  = {0, 0, m_rhi->getSwapchainInfo().extent.width, m_rhi->getSwapchainInfo().extent.height};

        m_rhi->cmdBindPipeline(m_rhi->getCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_render_pipelines[0].pipeline);
        m_rhi->cmdSetViewport(0, 1, &viewport);
        m_rhi->cmdSetScissor(0, 1, &scissor);
